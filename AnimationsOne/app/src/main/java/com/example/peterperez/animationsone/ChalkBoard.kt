package com.example.peterperez.animationsone
...
(context: Context) : View(context) {
    ...
    fun wander() {
        ...
        when (style) {
            ...
            BOOMARANG ->{
                val moving = getObjectAnimator(500, "fraction", 0.0f, 1.0f)
                val spinner = getObjectAnimator(10000, "angle", 0.0f, 360.0f)
                val spinMove = AnimatorSet()
                val recall = getObjectAnimator(500,"fraction", 1.0f,0.0f)
                val reverse = getObjectAnimator(1000, "angle", 360.0f, 0.0f)
                val duck = AnimatorSet()
                spinMove.play(moving).with(spinner)
                spinMove.start()
                duck.play(recall).with(reverse)
                duck.start()
            }

            SPIN_AND_SLIDE -> {
                val moveguy = getObjectAnimator(2000, "fraction", 0.0f, 1.0f)
                val recolorguy = getObjectAnimator(500, "currColor", 0.0f, 1.0f)
                val spinguy = getObjectAnimator(700, "angle", 0.0f, 360.0f)
                val atOnce = AnimatorSet()
                atOnce.play(moveguy).after(spinguy)
    ...
    companion object {  //static stuff
        ...
        const val BOOMARANG =  24
        const val SHOOT_OFF =25
        const val SPIN_AND_SLIDE =  26
    }
}